...
from app_pages.helpers.macro.gdp_functions import load_banco_annual, variable_growth, gdp_per_capita_growth
from app_pages.helpers.macro.job_market_functions import load_desestacionalizado_unemployment
from app_pages.helpers.macro.ise_functions import ise_long
...
from app_pages.helpers import kpi_cards as kc
from app_pages.helpers.miscellaneous import rates_functions as rf
from generalities.i18n import t, fmt_num, fmt_date
...
gdp_cfg = {
    ...
    "value": f"{fmt_num(annual_growth, '.2f')}%",
    "delta_text": f"Q{gdp_quarter} {gdp_year}: {fmt_num(quarter_growth['Growth'], '.2f')}%",
    "delta_good": quarter_growth["Growth"] >= 0,
    "metadata": f"{latest_year} · {t('Annual, vs prior quarter')}",
    "accent": "#4B7EC0",
...
if year_ago_date in cpi.index:
    ...
    cpi_delta = f"{fmt_num(delta_pp, '+.2f')} {t('pp vs')} {fmt_date(year_ago_date, '%b %Y')}"
    cpi_delta_good = delta_pp < 0
last_updated_dates.append(cpi_date)

cpi_cfg = {
    "title": "Annual Inflation",
    "value": f"{fmt_num(cpi_value, '.2f')}%",
    "delta_text": cpi_delta,
    "delta_good": cpi_delta_good,
    "metadata": f"{fmt_date(cpi_date, '%B %Y')} · {t('Monthly, YoY')}",
    "accent": "#f97316",
...
if year_ago_date in unemployment.index:
    ...
    unemp_delta = f"{fmt_num(delta_pp, '+.2f')} {t('pp vs')} {fmt_date(year_ago_date, '%b %Y')}"
    unemp_delta_good = delta_pp < 0
last_updated_dates.append(unemp_date)

unemployment_cfg = {
    "title": "Unemployment Rate",
    "value": f"{fmt_num(unemp_value, '.2f')}%",
    "delta_text": unemp_delta,
    "delta_good": unemp_delta_good,
    "metadata": f"{fmt_date(unemp_date, '%B %Y')} · {t('Monthly, YoY')}",
    "accent": "#16a34a",
...
if not ise_prev_year_row.empty:
    ...
    ise_delta = f"{fmt_num(delta_pp, '+.2f')} {t('pp vs')} {t(months[ise_latest['Mes']])} {ise_latest['Fecha'] - 1}"
    ise_delta_good = delta_pp >= 0

ise_cfg = {
    "title": "ISE Annual Growth",
    "value": f"{fmt_num(ise_latest['Valor'], '.2f')}%",
    "delta_text": ise_delta,
    "delta_good": ise_delta_good,
    "metadata": f"{t(months[ise_latest['Mes']])} {ise_latest['Fecha']} · {t('Annual, YoY')}",
    "accent": "#4f46e5",
...
if pd.notna(month_ago_value):
    trm_delta = f"{fmt_num(trm_value - month_ago_value, '+.2f')} COP {t('vs 1 month ago')}"
last_updated_dates.append(trm_date)

exchange_rate_cfg = {
    "title": "Exchange Rate",
    "value": f"{fmt_num(trm_value, ',.2f')} COP",
    "delta_text": trm_delta,
    "delta_good": None,
    "metadata": f"{fmt_date(trm_date, '%d %b %Y')} · {t('Daily')}",
    "accent": "#06b6d4",
...
policy_rate_cfg = {
    ...
    "value": f"{fmt_num(policy_value, '.2f')}%",
    "delta_text": f"{fmt_num(policy_value - prev_value, '+.2f')} {t('pts vs')} {fmt_date(prev_date, '%d %b %Y')}",
    "delta_good": None,
    "metadata": f"{fmt_date(policy_date, '%d %b %Y')} · {t('Rate changes')}",
    "accent": "#9333ea",
...
if len(misery_series) > 1:
    ...
    misery_delta = f"{fmt_num(delta, '+.2f')} {t('pts vs')} {misery_series.index[-2]}"
    misery_delta_good = delta < 0

misery_cfg = {
    "title": "Misery Index",
    "value": fmt_num(misery_value, ".2f"),
    "delta_text": misery_delta,
    "delta_good": misery_delta_good,
    "metadata": f"{misery_year} · {t('Annual, 2× Seasonally Adj. Unemployment + Inflation + Lending Rate − GDP per-capita growth')}",
    "accent": "#ea580c",
...
if str(prev_year) in gdp_nominal.index:
    ...
    if not prev_rows.empty:
        ...
        debt_delta = f"{fmt_num(debt_pct - prev_pct, '+.2f')} {t('pp vs')} {fmt_date(debt_latest_row.name, '%b')} {prev_year}"
...
debt_gdp_cfg = {
    ...
    "value": f"{fmt_num(debt_pct, '.2f')}%",
    "delta_text": debt_delta,
    "delta_good": debt_delta_good,
    "metadata": f"{fmt_date(debt_latest_row.name, '%b %Y')} · {t('Monthly, YoY')}",
    "accent": "#dc2626",
...
if len(productivity_series) > 1:
    ...
    prod_delta = f"{fmt_num(delta_pp, '+.2f')} {t('pp vs')} {productivity_series.index[-2]}"
    prod_delta_good = delta_pp >= 0

productivity_cfg = {
    "title": "Labor Productivity Growth",
    "value": f"{fmt_num(prod_value, '.2f')}%",
    "delta_text": prod_delta,
    "delta_good": prod_delta_good,
    "metadata": f"{prod_year} · {t('Annual, per employed person')}",
    "accent": "#0d9488",
...
if len(wage_series) > 1:
    ...
    wage_delta = f"{fmt_num(growth_pct, '+.2f')}% vs {wage_series.index[-2]}"
    wage_delta_good = growth_pct >= 0

minimum_wage_cfg = {
    "title": "Minimum Wage",
    "value": f"{fmt_num(wage_value, ',.0f')} COP",
    "delta_text": wage_delta,
    "delta_good": wage_delta_good,
    "metadata": f"{wage_year} · {t('Annual')}",
    "accent": "#d97706",
...
def _indicator_cfg(title, column, accent, value_suffix):
    ...
    if year_ago in series.index:
        ...
        delta_text = f"{fmt_num(delta, '+.2f')} vs {fmt_date(year_ago, '%b %Y')}"
        delta_good = None
    return {
        "title": title,
        "value": fmt_num(value, ".2f") + t(value_suffix),
        "delta_text": delta_text,
        "delta_good": delta_good,
        "metadata": f"{fmt_date(date, '%b %Y')} · {t('Monthly, YoY')}",
        "accent": accent,
        "spark": series.tail(12),
    }


debt_duration_cfg = _indicator_cfg("Debt Duration", "Duración - Deuda total", "#dc2626", " yrs")
debt_avg_life_cfg = _indicator_cfg("Debt Average Life", "Vida media - Deuda total", "#dc2626", " yrs")
debt_avg_coupon_cfg = _indicator_cfg("Debt Average Coupon", "Cupón promedio (%) - Deuda Total", "#dc2626", "%")
...
fiscal_deficit_cfg = {
    ...
    "value": f"{fmt_num(deficit_value, '.2f')}%",
    "delta_text": f"{fmt_num(deficit_delta, '+.2f')} {t('pp vs')} {deficit_series.index[-2]}",
    "delta_good": deficit_delta > 0,
    "metadata": f"{deficit_series.index[-1]} · {t('Annual, GNC (preliminary)')}",
    "accent": "#dc2626",
...
def _poverty_cfg(cfg):
    ...
    if len(series) > 1:
        ...
        delta_text = fmt_num(delta, cfg["delta_fmt"][0]) + cfg["delta_fmt"][1] + f" vs {series.index[-2]}"
        delta_good = delta <= 0
    return {
        "title": cfg["title"],
        "value": fmt_num(value, cfg["value_fmt"][0]) + cfg["value_fmt"][1],
        "delta_text": delta_text,
        "delta_good": delta_good,
        "metadata": f"{series.index[-1]} · {t('Annual')}",
        "accent": cfg["accent"],
        "spark": series.tail(12),
    }
...
if len(population_series) > 1:
    ...
    pop_delta = f"{fmt_num(growth_pct, '+.2f')}% vs {population_series.index[-2]}"
    pop_delta_good = None

population_cfg = {
    "title": "National Population",
    "value": fmt_num(pop_value, ",.0f"),
    "delta_text": pop_delta,
    "delta_good": pop_delta_good,
    "metadata": f"{pop_year} · {t('Annual')}",
    "accent": "#475569",
...
if len(births_series) > 1:
    ...
    births_delta = f"{fmt_num(growth_pct, '+.2f')}% vs {births_series.index[-2]}"
    births_delta_good = growth_pct >= 0

births_cfg = {
    "title": "Births (National)",
    "value": fmt_num(births_value, ",.0f"),
    "delta_text": births_delta,
    "delta_good": births_delta_good,
    "metadata": f"{births_year} · {t('Annual')}",
    "accent": "#db2777",
...
if len(deaths_series) > 1:
    ...
    deaths_delta = f"{fmt_num(growth_pct, '+.2f')}% vs {deaths_series.index[-2]}"
    deaths_delta_good = growth_pct <= 0

deaths_cfg = {
    "title": "Deaths (National)",
    "value": fmt_num(deaths_value, ",.0f"),
    "delta_text": deaths_delta,
    "delta_good": deaths_delta_good,
    "metadata": f"{deaths_year} · {t('Annual')}",
    "accent": "#57534e",
...
if len(migration_series) > 1:
    ...
    migration_delta = f"{fmt_num(delta, '+,.0f')} vs {migration_series.index[-2]}"
    migration_delta_good = None

net_migration_cfg = {
    "title": "Net Migration",
    "value": fmt_num(migration_value, ",.0f"),
    "delta_text": migration_delta,
    "delta_good": migration_delta_good,
    "metadata": f"{migration_year} · {t('Annual')}",
    "accent": "#0284c7",
...
with header_right:
    st.markdown(
        ...
        f'{t("Last updated")}<br><strong>{fmt_date(max(last_updated_dates), "%d %b %Y")}</strong></div>',
        unsafe_allow_html=True,
    )
...
kc.render_section({
    ...
    "page": "app_pages/Macroeconomics.py",
})
