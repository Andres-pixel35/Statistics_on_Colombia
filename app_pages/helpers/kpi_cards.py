...
def render_kpi_card(col, cfg: dict, key: str = None) -> None:
    ...
    with col, st.container(border=True):
        st.markdown(
            f'<span style="color:{cfg["accent"]};font-size:14px;font-weight:600;">{t(cfg["title"])}</span>',
...
def render_section(section: dict) -> None:
    ...
    with st.expander(t(section["title"]), expanded=section.get("expanded", False)):
        st.caption(t(section["description"]))
        render_kpi_grid(section["cfgs"], key_prefix=section["title"])
        if section.get("page"):
            st.page_link(section["page"], label=t("View full dashboard"))
