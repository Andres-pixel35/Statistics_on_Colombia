...
def deaths_gender_cause_pivot(dept_df: pd.DataFrame, cause: str) -> tuple:
    ...
    out.index.name = "Year"
    return out, [f"{t('Deaths by gender')} — {t(cause)}", "Year", "Deaths"]
...
def deaths_age_pivot(area_df: pd.DataFrame, gender: str = "Total", area: str = "Total") -> tuple:
    ...
    parts = [t(p) for p in (None if area == "Total" else area, None if gender == "Total" else gender) if p]
    title = t("Deaths by age group") + (f" — {', '.join(parts)}" if parts else "")
...
def deaths_age_cause_pivot(dept_df: pd.DataFrame, gender: str = "Total", cause: str = "All causes") -> tuple:
    ...
    out.index = out.index.astype(int)
    out.index.name = "Year"
    title = t("Deaths by age group") + (f" — {t(gender)}" if gender != "Total" else "")
    if cause != "All causes":
        title += f" ({t(cause)})"
    return out, [title, "Year", "Deaths"]
