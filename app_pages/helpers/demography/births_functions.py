...
def births_gender_age_pivot(age_df: pd.DataFrame, age_label: str) -> tuple:
    ...
    df.index = df.index.astype(int)
    df.index.name = "Year"
    return df, [t("Births by gender — mothers {age}").format(age=t(age_label)), "Year", "Births"]
...
def births_age_pivot(age_df: pd.DataFrame, gender: str = "Total") -> tuple:
    ...
    title = t("Births by mother's age group") + ("" if gender == "Total" else f" ({t(gender)})")
...
def births_education_pivot(edu_df: pd.DataFrame, age_label: str | None) -> tuple:
    ...
    title = t("Births by mother's education level")
