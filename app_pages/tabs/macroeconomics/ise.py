...
from app_pages.helpers.macro import ise_functions as isf
...
def render_ise(df: pd.DataFrame) -> None:
    ...
    with dataset_col:
        dataset = st.selectbox(t("Dataset:"), list(ig.DATASETS.keys()), format_func=t)

    paths = ig.ise_paths(dataset)
    units = ig.ise_units(dataset)
    growth_label = next(iter(paths))
    with metric_col:
        metric = st.selectbox(t("Metric:"), list(paths.keys()), format_func=t)
    ...
    category_label = st.sidebar.selectbox(
        t("Category:"), list(ig.CATEGORY_EN.values()), disabled=compare_categories, format_func=t
    )
    ...
    if category_sp == "Actividades terciarias" and not compare_categories:
        activity_labels = st.sidebar.multiselect(
            t("Activity:"), list(activity_dict.values()),
            default=[next(iter(activity_dict.values()))], key="ise_activities", format_func=t,
        )
    else:
        activity_labels = [st.sidebar.selectbox(
            t("Activity:"), list(activity_dict.values()), disabled=compare_categories, format_func=t
        )]
    ...
    with st.sidebar:
        ...
        st.checkbox(t("Compare Primary vs Secondary vs Tertiary"), key="ise_compare_categories")
    ...
    if compare_categories:
        ...
        if year_set:
            ...
            info = [f"{t('Primary vs Secondary vs Tertiary')} · {year}", "Month", unit]
        else:
            ...
            info = [f"{t('Primary vs Secondary vs Tertiary')} · {t(month_label)}", "Year", unit]
    elif not activities_sp:
        st.info(t("Select at least one activity."))
        return
    elif year_set:
        ...
        if comparing_activities:
            ...
            if len(years_sorted) >= 1 and activity_labels[0] != category_label:
                title = f"{t(category_label)} — {t(activity_labels[0])}"
            else:
        ...
        info = [f"{t(category_label)} · {t(month_label)}", "Year", unit]
    ...
    if metric == "Index":
        st.caption(f"{t('Metric:')} {t(metric)} ({t(unit)})")
    st.caption(t("Source: DANE — Indicador de Seguimiento a la Economía (ISE), 9-actividades breakdown"))
