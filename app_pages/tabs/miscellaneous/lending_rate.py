...
def render_lending_rate(df: pd.DataFrame) -> None:
    ...
    with col1:
        cur_years = st.multiselect(t("Year:"), year_options, key="lending_rate_years", disabled=yearpres_disabled)
    with col2:
        cur_month_labels = st.multiselect(t("Month:"), month_labels, default=["December"],
                                           key="lending_rate_months", disabled=month_disabled, format_func=t)
    ...
    if series.empty:
        ...
        mc.render_chart(fig)
        if annual_avg:
            st.caption(t("Using annual average"))
