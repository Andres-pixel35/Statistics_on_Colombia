...
def render_exchange_rate(df: pd.DataFrame) -> None:
    ...
    if in_month_view:
        ...
        series = rf.rate_day_compare(local, SPEC, day, month_num)
        ...
        col1, col2 = st.columns(2)
        with col1:
            cur_years = st.multiselect(t("Year:"), year_options, key="trm_years", disabled=yearpres_disabled)
        with col2:
            cur_month_labels = st.multiselect(t("Month:"), month_labels, key="trm_months", disabled=month_disabled, format_func=t)
    ...
    st.sidebar.checkbox(t("In-month view"), key="trm_in_month", disabled=day_compare)
    st.sidebar.checkbox(t("Day comparison"), key="trm_day_compare", disabled=in_month_view)
