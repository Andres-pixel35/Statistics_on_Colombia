...
def render_productivity(prod_df: pd.DataFrame) -> None:
    ...
    if stem == ACTIVITY_STEM:
        ...
        concept_labels = st.sidebar.multiselect(
            ...
            on_change=_cap, args=(concept_key, [activity_key]))
        if not concept_labels:
            concept_labels = [list(terms.keys())[0]]
        activity_labels = st.sidebar.multiselect(
            "Economic Activity:", list(pr.ACTIVITY_EN.values()), default=["Total Economy"],
            key=activity_key, on_change=_cap, args=(activity_key, [concept_key]))
