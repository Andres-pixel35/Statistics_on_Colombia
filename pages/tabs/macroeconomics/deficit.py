...
def render_deficit() -> None:
    ...
    with col1:
        frequency = st.selectbox("Frequency:", list(dg.FREQUENCIES), key="deficit_frequency")
    with col2:
        unit = st.selectbox("Unit:", list(dg.UNITS), key="deficit_unit")
    ...
    chart_type = st.sidebar.selectbox("Chart Type:", ["Line", "Bar", "Stacked bar"],
                                      key="deficit_chart_type")

    roots = [k for k in meta.index if meta.at[k, "depth"] == 0]
    group_key = st.sidebar.selectbox(
        "Group:", ["Headline"] + roots,
        format_func=lambda k: "Headline" if k == "Headline" else terms.get(k, k),
    ...
    concepts = st.sidebar.multiselect(
        "Concepts:", members, default=members[:1],
        format_func=lambda k: ("· " * meta.at[k, "depth"]) + terms.get(k, k),
    ...
    compare = st.sidebar.checkbox("Compare revenue vs. spending", key="deficit_compare")
    ...
    with col3:
        if frequency == "Monthly":
            ...
            period_label = st.selectbox(
                "Month:", month_labels, index=month_labels.index(months[12]),
                key="deficit_period", disabled=bool(year_set),
            )
        elif frequency == "Quarterly":
            period_label = st.selectbox(
                "Quarter:", ["Q1", "Q2", "Q3", "Q4"], index=3,
                key="deficit_period", disabled=bool(year_set),
            )
    ...
    group_title = "Headline" if group_key == "Headline" else terms.get(group_key, group_key)
    ...
    if compare:
        ...
        if frequency == "Annual":
            ...
            title = "Revenue vs. Spending" + (f" · {year}" if year else "")

    else:
        labels = [terms.get(k, k) for k in concepts]
        ...
        if frequency == "Annual":
            ...
            if years_sorted:
                ...
                title = f"{labels[0] if single else group_title} · {period_label}"
    ...
    st.caption("Shows the Central National Government (GNC) fiscal balance. A negative value is a deficit.")
    if unit == "% of GDP":
        st.caption("% of GDP is computed by the source against DANE nominal GDP.")
    if frequency == "Annual" and 2025 in years_sorted:
        st.caption("2025 figures are preliminary.")
    if chart_type == "Stacked bar":
        st.caption("Stacking is only meaningful within one group — mixing a total with its own "
